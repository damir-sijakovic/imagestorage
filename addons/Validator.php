...
class Validator
{
    ...
    public function check($data, $types)
    {
        ...
        {
            $msg = self::validate($data[$i], $types[$i], $i);
            if (isset($msg['fail'])){
                return $msg;
            }            
        }
...
//print_r( Validator::check(['chars', 2, 2.3], ['string', 'int', 'float']) );
//print_r( Validator::check(['dsfsdfds', 2, 2.4], ['string:122:under', 'int', 'float']) );
